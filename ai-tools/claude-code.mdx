---
icon: "asterisk"
title: "🚀 Building Apps with Pica + InstantDB is Easier Than Ever"
---
...
👉 **we’ll build a waiting list app** with real-time signups and automatic welcome emails.
...
That’s the magic of **Pica \+ InstantDB**.

> 💡 Full demo app (including an admin panel with live stats & signup table) is available here: GitHub Repo
...
// This gives you a real-time database client
...
export async function POST(request: NextRequest) {
  ...
  // Build a raw MIME email (Gmail expects this format)
  const mimeEmail = [
  ...
  // Send email via Pica → Gmail passthrough
  const response = await fetch(
    "<https://api.picaos.com/v1/passthrough/users/me/messages>",
    {
      ...
      headers: {
        ...
        "x-pica-action-id":
          "conn_mod_def::F_JeJ3qaLEg::v9ICSQZxR0un5_ketxbCAQ", // Gmail send action
  ...
  if (!response.ok) {
    // If Gmail rejects the request, return error
...
export default function EmailSignup() {
  ...
  // Live query → updates instantly when someone signs up
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const signupId = id(); // Generate unique ID

      // Save signup to InstantDB
      await db.transact([
    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 p-4">
      <div className="w-full max-w-md">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          ...
          {status === "error" && (
            <p className="mt-2 text-red-700 text-center">
...
// Home page → shows waiting list form
...
With Pica \+ InstantDB, you can just as easily build:

- A CRM that emails new leads
- A task app that sends Slack reminders
- A dashboard that syncs with Notion

The possibilities go way beyond waiting lists.

👉 [Try Pica today](https://picaos.com/) and see how much faster you can ship full-stack apps and bring showcase your ideas to the world.

👉 Check out the **full demo repo with an admin panel**: [GitHub Repo](https://github.com/example/pica-instantdb-waitlist-demo)

Bonus: Add an Awesome Admin panel 📂 `src/app/AdminDashboard.tsx`
...
Ensure to add your admin panel on the main file
